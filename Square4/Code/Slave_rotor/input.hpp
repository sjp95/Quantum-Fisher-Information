...
class input
{
    private:
        ...
        int le = pow(Ls,N);
        int Nf =20;                  // Lattice diamention            | L
        double mus = 0.0;           // chemical potential of Hs       | T
        double muf=0;               // Fermionic chemical potential   |
        double lambda=0.0;
        int ls=0;
        double filling =1;
        ...
        MatrixXcd J= MatrixXcd::Ones(N,N);             //             | D 
        // MatrixXcd t= MatrixXcd::Ones(N,N);             //             | D 
        // MatrixXcd X= MatrixXcd::Ones(N,N);             //             | A
        // MatrixXcd X1= MatrixXcd::Ones(N,N);             //             | A
        // MatrixXcd X2= MatrixXcd::Ones(N,N);             //             | A
        // MatrixXcd nn= MatrixXcd::Ones(N,N);            //             | T
    ...
    public:               
        ...
        void besis(int NN1,double J0,double hx0, double hy0, double hz0);
        void Values();
