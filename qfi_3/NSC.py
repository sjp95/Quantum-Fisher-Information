...
if file.exists():
    ...
    c=plt.imshow(zi, extent=[x.min(),x.max(), 0, y.max()], origin='lower',cmap='jet',aspect=3*(x.max()-x.min())/y.max(),interpolation='catrom') #gist_ncar
    ...
    plt.text(min(x)+12*(max(x)-min(x))/36, max(y)-1*max(y)/8, r'$J_x=$ '+s0+r'$J_y$',fontsize=45,color='white')
#=======================================C-1==========================================#
#=======================================C-2==========================================#
# N = 100
...
ff1 =str("Plot/NSC/Order_"+s+"_"+NNN+"_"+hz+".pdf")#
